...
module.exports = projectName =>
  async function() {
    ...
    let localVersion =
      projectName === "dio-cli" ? cliPackageJson.version : await getActualVersion(projectName);
    ...
    return new Promise((resolve, reject) => {
      if (!semver.satisfies(process.version, cliPackageJson.engines.node)) {
        spinner.stop();
        log(`    你必须升级Node版本至 >=${cliPackageJson.engines.node}.x 才能使用dio-cli`, "red");
        reject();
      }

      http
        .get(`https://registry.npmmirror.com/${projectName}`, res => {
          ...
          res.on("end", () => {
            ...
            if (semver.gtr(localVersion, latestVersion)) {
              ...
              log();
              log(`  你可以在结束后输入 'dio update' 来更新 ${projectName}`, "white");
              log();
              logBoundary("yellow");
              log();
            }
