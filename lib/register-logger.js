...
module.exports = (cmd, process) => {
  ...
  const file = path.posix.join(__dirname, "..", "dio-cli.log");
  ...
  process.on("uncaughtException", err => {
    fs.writeFileSync(
      file,
      `[Error]Dio${cmd}:${process.exitCode} (${new Date().toLocaleString()})\n` + `${err.stack}\n`,
      { flag: "a" }
    );
    throw err;
  });
  process.on("exit", code => {
    ...
    fs.writeFileSync(file, `[${state}]Dio${cmd}:${code} (${new Date().toLocaleString()})\n`, {
      flag: "a"
    });
  });
};
