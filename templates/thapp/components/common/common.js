...
function getUrlparams() {
    ...
    if (arr !== null) {
        for (var i = 0, l = arr.length; i < l; i++) {
            ...
            if (value && value.length > 1) { returnObj[decodeURIComponent(value[0])] = value[1] }
        }
    }
    return returnObj;
};
 ...
 * @returns
 */
function getParamValue(name, type = 'on') {
    var param = getUrlparams();
    if (param[name]) {
        if (type == 'en') {
            return dioE.unCompile(param[name]);
        } else {
            return decodeURIComponent(param[name]);
        }
    }
    return null;
}
 ...
 * @param {string} [type='url'] 
 */
function getValueEncrypt(key, type = 'url') {
    switch (type) {
        case "url":
            return getParamValue(key, 'en');
            break;
    }
}
...
module.exports = {
    ...
    getValue: getValue,
    getValueEncrypt: getValueEncrypt,
